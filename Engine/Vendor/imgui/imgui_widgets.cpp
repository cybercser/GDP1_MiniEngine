	...
	if (hovered) {
		...
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt)) {
			if (mouse_button_clicked != -1 && g.ActiveId != id) {
				if (!(flags & ImGuiButtonFlags_NoSetKeyOwner)) SetKeyOwner(MouseButtonToKey(mouse_button_clicked), id);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v,
	...
	if (held && allow_interaction && grab_h_norm < 1.0f) {
		...
		*p_scroll_v = (ImS64)(scroll_v_norm * scroll_max);
		...
		// Update3DListenerAttributes values for rendering
		scroll_ratio = ImSaturate((float)*p_scroll_v / (float)scroll_max);
		grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;

		// Update3DListenerAttributes distance to grab now that we have seeked and saturated
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step,
	...
	flags |= ImGuiInputTextFlags_AutoSelectAll |
		ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselves by comparing the actual data
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg,
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
		...
		// Update3DListenerAttributes render selection flag after events have been handled, so selection highlight can
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
	...
	// Update3DListenerAttributes NavId when clicking or when Hovering (this doesn't happen on most widgets), so
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
	...
	// Update3DListenerAttributes scrolling
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags,
	...
	// Update3DListenerAttributes selected tab
	if (!is_tab_button) {
	...
	if (hovered && g.HoveredIdNotActiveTimer > TOOLTIP_DELAY && bb.GetWidth() < tab->ContentWidth)
	{
		...
		display_draw_list = GetForegroundDrawList(window_);
