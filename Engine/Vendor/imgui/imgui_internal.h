...
#pragma clang diagnostic ignored "-Wfloat-equal"      // warning: comparing floating point with == or != is unsafe //
                                                      // storing and comparing against same constants ok, for ImFloor()
...
struct ImGuiInputTextDeactivateData;  // Short term storage to backup text of a deactivating InputText() while another
...
#ifdef _WIN32
#define IM_NEWLINE \
    "\r\n"  // Play it nice with Windows users (Update3DListenerAttributes: since 2018-05, Notepad finally appears to
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_NoNav = 1 << 3,         // false     // Disable any form of focusing (keyboard/gamepad directional
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    ImGuiButtonFlags_NoSetKeyOwner = 1 << 20,     // don't set key/input owner on the initial click (note: mouse buttons
...
enum ImGuiSelectableFlagsPrivate_ {
    ...
    ImGuiSelectableFlags_NoSetKeyOwner = 1
                                         << 27,  // Don't set key/input owner on the initial click (note: mouse buttons
                                                 // are keys! often, the key in question will be ImGuiKey_MouseLeft!)
};
...
struct IMGUI_API ImGuiMenuColumns {
    ...
    ImU16 OffsetLabel;  // Offsets are locked in Update3DListenerAttributes()
...
struct ImGuiPopupData {
    ...
    int ParentNavLayer;    // Resolved on BeginPopup(). Actually a ImGuiNavLayer type (declared down below), initialized
                           // to -1 which is not part of an enum, but serves well-enough as "not any of layers" value
...
struct ImGuiInputEvent {
    ...
    ImU32 EventId;  // Unique, sequential increasing integer to identify an event (if you need to correlate them to
                    // other data).
    union {
...
enum ImGuiInputFlags_ {
    ...
    ImGuiInputFlags_LockUntilRelease =
        1 << 7,  // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for
                 // polling). Cleared when the key is released or at end of each frame if key is released. This is
                 // useful to make input-owner-aware code steal keys from non-input-owner-aware code.
    ...
    ImGuiInputFlags_RouteFocused =
        1 << 8,  // (Default) Register focused route: Accept inputs if window is in focus stack. Deep-most focused
    ...
    ImGuiInputFlags_RouteGlobal = 1 << 10,      // Register route globally (medium priority: unless an active item
    ...
    ImGuiInputFlags_RouteUnlessBgFocused =
        1 << 13,  // Global routes will not be applied if underlying background/void is focused (== no Dear ImGui
...
struct ImGuiViewportP : public ImGuiViewport {
    ...
    }  // Update3DListenerAttributes public fields
...
struct ImGuiContext {
    ...
    ImGuiID DebugLocateId;  // Storage for DebugLocateItemOnHover() feature: this is read by ItemAdd() so we keep it in
                            // a hot/cached location
    ...
    ImGuiID ColorEditCurrentID;  // Set temporarily while inside of the parent-most ColorEdit4/ColorPicker4 (because
    ...
    float ColorEditSavedHue;     // Backup of last Hue associated to LastColor, so we can restore Hue in lossy RGB<>HSV
                                 // round trips
    float ColorEditSavedSat;     // Backup of last Saturation associated to LastColor, so we can restore Saturation in
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    ImVec2 CursorStartPosLossyness;  // Record the loss of precision of CursorStartPos due to really large scrolling
                                     // amount. This is used by clipper to compensate and fix the most common use case
...
struct IMGUI_API ImGuiWindow {
    ...
    ImGuiViewportP* Viewport;  // Always set in Begin(). Inactive windows may have a NULL value here if their viewport
                               // was discarded.
...
enum ImGuiTabBarFlagsPrivate_ {
    ...
    ImGuiTabBarFlags_SaveSettings = 1 << 22,  // FIXME: Settings are handled by the docking system, this only request
                                              // the tab bar to mark settings dirty when reordering tabs
};
...
struct IMGUI_API ImGuiTable {
    ...
    ImRect BgClipRect;  // We use this to cpu-clip cell background color fill, evolve during the frame as we cross
...
IMGUI_API void ActivateItemByID(
    ImGuiID id);  // Activate an item by ID (button, checkbox, tree node etc.). Activation is queued and processed on
...
IMGUI_API bool IsKeyPressed(
    ...
    ImGuiInputFlags flags =
        0);  // Important: when transitioning from old to new IsKeyPressed(): old API has "bool repeat = true", so would
             // default to repeat. New API requiress explicit ImGuiInputFlags_Repeat.
