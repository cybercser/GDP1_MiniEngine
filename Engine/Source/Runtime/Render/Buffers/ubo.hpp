...
enum class Type : unsigned char { SCALAR = 0, VEC2, VEC3, VEC4, ARRAY, STRUCT, INVALID };
...
inline Element newScalar() { return Element(); }
...
inline Element newArray(unsigned int length, Element arrElement) {
    ...
    ret.baseAlign = arrElement.type == Type::STRUCT ? arrElement.baseAlign : roundUpPow2(arrElement.baseAlign, 4);

    return ret;
}

inline Element newColMat(unsigned char cols, unsigned char rows) { return newArray(cols, newVec(rows)); }
...
inline Element newRowMat(unsigned char rows, unsigned char cols) { return newArray(rows, newVec(cols)); }
...
public:
    ...
    */
    ...
    // generate object
    void generate() { glGenBuffers(1, &val); }

    // bind object
    void bind() { glBindBuffer(type, val); }
    ...
    void clear() { glBindBuffer(type, 0); }

    // cleanup
    void cleanup() { glDeleteBuffers(1, &val); }
};
...
public:
    ...
    unsigned int calcSize() { return block.calcPaddedSize(); }
