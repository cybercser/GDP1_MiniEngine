...
std::vector<DriverInfo> AudioManager::GetDeviceDrivers() {
    ...
    for (int i = 0; i < numDrivers; i++) {
        ...
        FMOD_RESULT result =
            m_System->getDriverInfo(driverInfo.id, driverInfo.name, 256, &driverInfo.guid, &driverInfo.systemrate,
                                    &driverInfo.speakermode, &driverInfo.speakermodechannels);

        // Print to console
        printf("Device %d: \n name: %s\n guid: %d %d %d\n systemrate: \n speakermode: %d\n channels: %d\n",
               driverInfo.id, driverInfo.name, driverInfo.guid.Data1, driverInfo.guid.Data2, driverInfo.guid.Data3,
               driverInfo.systemrate, (int)driverInfo.speakermode, driverInfo.speakermodechannels);
        drivers.push_back(driverInfo);
    }
    return drivers;
}

void AudioManager::SetDeviceDriver(int id) { m_System->setDriver(id); }
...
void AudioManager::Load3DAudio(const char* file, float minDistance, float maxDistance) {
    if (!m_Initialized) return;
    ...
    m_AudioMap[file]->Audio->set3DMinMaxDistance(minDistance, maxDistance);
...
int AudioManager::PlayAudio(const char* AudioName, int channelId, glm::vec3 soundPos, glm::vec3 soundVel) {
    // Ensure the Audio Manager is initialized before we do anything
    if (!m_Initialized) {
    ...
    FMOD_RESULT result = m_System->playSound(it->second->Audio,  // The sound to play
    ...
    FMOD_VECTOR sPos;
    GLMToFMOD(soundPos, sPos);
    FMOD_VECTOR sVel;
    GLMToFMOD(soundVel, sVel);

    result = channel->fmodChannel->set3DAttributes(&sPos, &sVel);
    FMODCheckError(result);
...
/** ECHO PASS FILTER DSP EFFECT **/

void AudioManager::AddEchoPassOnChannel(int channelId) {
    FMOD_RESULT result = m_System->createDSPByType(FMOD_DSP_TYPE_ECHO, &m_EchoPassDSP);
    FMODCheckError(result);

    m_EchoPassDSP->setParameterFloat(FMOD_DSP_ECHO_DELAY, 1.f);
    m_EchoPassDSP->setParameterFloat(FMOD_DSP_ECHO_DRYLEVEL, 0.8f);
    m_EchoPassDSP->setParameterFloat(FMOD_DSP_CHORUS_DEPTH, 3.f);
    FMODCheckError(result);

    m_ChannelList[channelId]->fmodChannel->addDSP(4, m_EchoPassDSP);
}
...
void AudioManager::SetChannel3DAttributes(int channelId, glm::vec3& position, glm::vec3& velocity) {
    Channel* channel = m_ChannelList[channelId];

    FMOD_VECTOR sPos;
    GLMToFMOD(position, sPos);
    FMOD_VECTOR sVel;
    GLMToFMOD(velocity, sVel);

    FMOD_RESULT result = channel->fmodChannel->set3DAttributes(&sPos, &sVel);
    FMODCheckError(result);

    result = channel->fmodChannel->set3DMinMaxDistance(0.5f, 15.0f);
    FMODCheckError(result);
}
...
void AudioManager::SetListenerAttributes(const glm::vec3& position, const glm::vec3& velocity, const glm::vec3& forward,
                                         const glm::vec3& up) {
...
void AudioManager::GetListenerAttributes(glm::vec3& position, glm::vec3& velocity, glm::vec3& forward, glm::vec3& up) {
    FMOD_VECTOR fmodPosition;
    FMOD_VECTOR fmodVelocity;
    FMOD_VECTOR fmodForward;
    FMOD_VECTOR fmodUp;

    FMOD_RESULT result = m_System->get3DListenerAttributes(0, &fmodPosition, &fmodVelocity, &fmodForward, &fmodUp);
    FMODCheckError(result);
...
int AudioManager::AddPolygon(float direct, float reverb, bool doublesided, const std::vector<glm::vec3>& vertices,
